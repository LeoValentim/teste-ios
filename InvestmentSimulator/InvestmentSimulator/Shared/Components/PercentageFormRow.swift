...
// MARK: - PercentageFormRow
class PercentageFormRow: UIView {
    // MARK: Properties
    private(set) var title: String = ""
    private(set) var placeholder: String = ""
    private(set) var isRequired: Bool = false
    public var didChangeText: ((String?) -> Void)?
    private let percentageFormatter = SumTextInputFormatter(textPattern: "###%")
    private let percentageInputController = PercentageTextFieldInputController()
    // MARK: UI components
...
extension PercentageFormRow: ViewCodeConfiguration {
    func setupHierarchy() {
    ...
    func setupViews() {
        ...
        inputField.delegate = percentageInputController
        percentageInputController.formatter = percentageFormatter
        percentageInputController.didChangeText = { [weak self] text in
            let unformattText = self?.percentageFormatter.unformat(text)
            self?.didChangeText?(unformattText)
        }
        // BottomLine
...
class PercentageTextFieldInputController: NSObject, UITextFieldDelegate {
